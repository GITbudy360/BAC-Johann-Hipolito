...
# --- FIX 1: Remove the human pause. Wait programmatically for KEDA and Operator. ---
echo -e "Waiting for KEDA to scale the RedisCluster to at least 4 masters under load..."
for _ in $(seq 1 300); do
    leaders=$(k3s kubectl get statefulset redis-cluster-leader -n redis -o jsonpath='{.status.readyReplicas}' 2>/dev/null || echo 0)
    if [[ "$leaders" -ge 4 ]]; then
        echo -e "${GREEN}Detected operator scaling out to $leaders leader(s)!${NC}"
        break
    fi
    sleep 5
done
...
gate_settled() {
    ...
    while true; do
        ...
        # --- FIX 3: Surface Raw Telemetry (Eliminating the blindspot) ---
        echo -e "${YELLOW}--- RAW CLUSTER CHECK OUTPUT ---${NC}"
        echo "$chk"
        echo -e "${YELLOW}--------------------------------${NC}"
        ...
        if echo "$chk" | grep -q "are open"; then
            ...
            echo -e "Cluster not settled at ${want} masters yet. Inspect the raw output above."
        fi
...
# configEpoch split ("Nodes don't agree about configuration!") in earlier runs.
