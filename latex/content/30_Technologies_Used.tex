...
including \textit{StatefulSets}, \textit{CustomResourceDefinitions}, and Helm
...
\textit{StatefulSet} with a minimum of three primary nodes and forms the core
...
\textit{StatefulSet} in response to CPU and memory utilisation thresholds.
...
(\cite{keda:home}). KEDA introduces the \textit{ScaledObject} custom resource,
which declares a scaling target (such as a \textit{Deployment} or
\textit{StatefulSet}), a polling interval, cooldown periods, and one or more
...
\textit{kube-prometheus-stack} Helm chart, which bundles Prometheus, Grafana,
...
collecting cluster-level metrics from \textit{kube-state-metrics} and
\textit{node-exporter}, as well as application-level metrics from the FastAPI
...
\textit{kube-prometheus-stack}. In this thesis, Helm is used to deploy the
